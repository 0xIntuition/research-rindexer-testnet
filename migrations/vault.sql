-- Migration: Create vault materialized view
...
CREATE MATERIALIZED VIEW public.vault AS
WITH
...
latest_share_price AS (
    SELECT DISTINCT ON (term_id, curve_id)
        ...
        tx_index,
        block_timestamp
    FROM intuition_multi_vault.share_price_changed
...
SELECT
    ...
    CAST(lsp.tx_hash AS TEXT) AS transaction_hash,
    CAST(lsp.tx_index AS BIGINT) AS transaction_index,
    fe.created_at,
...
COMMENT ON MATERIALIZED VIEW public.vault IS
'Aggregated vault data from SharePriceChanged events. Shows current state including total assets, total shares, share price, market cap, and position count per (term_id, curve_id). Updated via refresh_vault_view().';

COMMENT ON FUNCTION refresh_vault_view() IS
'Refreshes the vault materialized view using CONCURRENT mode. Can be called manually or scheduled via pg_cron for periodic updates.';

COMMENT ON COLUMN public.vault.term_id IS
'Unique identifier for the term (atom or triple) as hex-encoded bytes32.';

COMMENT ON COLUMN public.vault.curve_id IS
...
COMMENT ON COLUMN public.vault.position_count IS
'Number of active positions (accounts with shares > 0) in this vault.';

COMMENT ON COLUMN public.vault.created_at IS
'Timestamp when the vault was first created (first SharePriceChanged event).';

COMMENT ON COLUMN public.vault.updated_at IS
